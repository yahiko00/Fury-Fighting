...
*/
	...
	{
		r = rot[i].x;
		obj->precalcul[i].p.x = ( float )cos( r );
		obj->precalcul[i].r.x = ( float )sin( r );

		r = rot[i].y;
		obj->precalcul[i].p.y = ( float )cos( r );
		obj->precalcul[i].r.y = ( float )sin( r );

		r = rot[i].z;
		obj->precalcul[i].p.z = ( float )cos( r );
		obj->precalcul[i].r.z = ( float )sin( r );
	}
...
float Al_Kashi( float ax, float bx, float cx, float ay, float by, float cy )
{
	...
	a = ( float )hypotenuse2D( ( ax - cx ), ( ay - cy ) );
	b = ( float )hypotenuse2D( ( bx - cx ), ( by - cy ) );
	c = ( float )hypotenuse2D( ( ax - bx ), ( ay - by ) );
